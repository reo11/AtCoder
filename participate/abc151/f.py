...
def circumcenter(x1, y1, x2, y2, x3, y3):
    ...
    if area == 0:
        ...
        m = max(ab, bc, ca)
        if m == ab:
            return (x1 - x2) / 2, (y1 - y2)/2, math.sqrt((x1 - x2)**2 + (y1 - y2)**2)
        elif m == bc:
            return (x2 - x3) / 2, (y2 - y3)/2, math.sqrt((x2 - x3)**2 + (y2 - y3)**2)
        else:
            return (x3 - x1) / 2, (y3 - y1)/2, math.sqrt((x3 - x1)**2 + (y3 - y1)**2)
...
if n > 2:
    for a, b, c in itertools.combinations(range(n), 3):
        cx, cy, cr = circumcenter(xy[a][0], xy[a][1], xy[b][0], xy[b][1], xy[c][0], xy[c][1])
        # check
        rr = cr ** 2
        flag = True
        for i in range(n):
